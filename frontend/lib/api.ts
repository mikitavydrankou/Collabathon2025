...
export class ApiClient {
	...
	async sendQAMessage(data: {
		...
		const response = await fetch(`${this.baseUrl}/qa/chat`, {
			method: 'POST',
			headers: this.getAuthHeaders(),
			body: JSON.stringify(data),
		})
		...
		if (!response.ok) {
			const error = await response.json()
			throw new Error(error.detail || 'Failed to send QA message')
		}

		return response.json()
	}

	async getQAHistory(sessionId: string): Promise<{
		session_id: string
		history: any[]
	}> {
		const response = await fetch(
			`${this.baseUrl}/qa/history/${sessionId}`,
			{
				method: 'GET',
				headers: this.getAuthHeaders(),
			}
		...
		if (!response.ok) {
			const error = await response.json()
			throw new Error(error.detail || 'Failed to get QA history')
		}
