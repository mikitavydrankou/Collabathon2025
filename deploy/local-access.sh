...
#
# kind has no cloud LB, so we port-forward the Envoy proxy Service (bound to
# 0.0.0.0 so the Windows host can reach it). On GKE the Gateway gets a real
# external IP and no port-forward is needed.
set -euo pipefail
